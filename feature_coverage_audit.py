...
# Fast deterministic QA gate: up to 3 companies per represented FF48 industry.
# This is NOT the inferential backtest sample and never sees forward outcomes.
MAX_PER_FF48=3
SEC_BATCH=24
...
def query_facts_batched(ciks):
    ...
    for offset in range(0,len(ciks),SEC_BATCH):
        ...
        for attempt in range(7):
            try:
                mirror=SECFinancialStatementMirror();df=mirror.con.execute(_facts_sql(batch)).df();frames.append(df)
                attempt_log.append({'offset':offset,'n':len(batch),'attempt':attempt+1,'rows':len(df),'status':'PASS'})
                time.sleep(1.0);last=None;break
            except Exception as e:
                last=e;msg=f'{type(e).__name__}: {e}';attempt_log.append({'offset':offset,'n':len(batch),'attempt':attempt+1,'status':'RETRY','error':msg[:300]});time.sleep(min(90,5*(2**attempt)))
        if last is not None:raise RuntimeError(f'SEC mirror batch failed after retries offset={offset}: {type(last).__name__}: {last}')
...
def load_price(ticker):
    ...
    for attempt in range(5):
        try:
            p=CachedEngo();h=p.raw_history(ticker,HISTORY_START,SIGNAL);a=p.actions(ticker);return ticker,h,a,None
        except Exception as e:
...
def main():
    ...
    facts,sec_attempts=query_facts_batched(sample.cik.tolist());facts['cik']=facts.cik.astype(str).str.zfill(10);facts.to_parquet(outdir/'sample_fsd_facts.parquet',index=False)
    ...
    for r in sample.itertuples():
        ...
        try:
            fr=fundamental_raw(q,actions.get(ticker),SIGNAL);base.update(fr);mr=market_raw(h,spy,SIGNAL,fr.get('shares_signal_raw',np.nan)) if not h.empty else {};base.update(mr)
            base['eligible']=bool(pd.notna(base.get('ADV60_raw')) and base['ADV60_raw']>=1_000_000);base['feature_error']=price_errors.get(ticker)
        except Exception as e:
    ...
    report={
      'status':'PASS' if len(scored) and scored.get('OFS_A_OPEN',pd.Series(dtype=float)).notna().any() else 'FAIL','qa_sample_max_per_ff48':MAX_PER_FF48,
      'signal_date':str(SIGNAL.date()),'identity_eligible_population':int(len(u)),'stratified_sample_rows':int(len(sample)),'ff48_industries_sampled':int(sample.industry_code.nunique()),'raw_rows':int(len(raw)),
      'sec_batches':int((len(sample)+SEC_BATCH-1)//SEC_BATCH),'sec_retry_count':int(sum(1 for a in sec_attempts if a['status']=='RETRY')),
      'eligible_adv60_rows':int(raw.get('eligible',False).sum()),'price_errors':len(price_errors),'feature_exceptions':len(exceptions),'aq_mature_rows':int(scored.get('AQ_raw',pd.Series(dtype=float)).notna().sum()),
      'share_history_suspect_rows':int(scored.get('share_history_suspect',pd.Series(dtype=bool)).fillna(False).sum()),'component_nonmissing_rates':{c:float(scored[c].notna().mean()) if c in scored else 0.0 for c in components},
      'scorable_rows':int(scored.get('scorable',pd.Series(dtype=bool)).fillna(False).sum()),'scorable_rate':float(scored.get('scorable',pd.Series(dtype=bool)).fillna(False).mean()) if len(scored) else 0.0,
      'feature_error_examples':list(dict.fromkeys([x for x in raw.get('feature_error',pd.Series(dtype=object)).dropna().astype(str)]))[:20],'NO_FORWARD_OUTCOMES_ACCESSED':True,
    }
